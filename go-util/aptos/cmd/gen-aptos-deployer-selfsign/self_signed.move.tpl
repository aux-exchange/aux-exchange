...
// Argumenets:{{.Args}}
// {{.ModuleName}} controls the signer capability of this module.
...
{{range .Friends}}    friend {{.Address}}::{{.ModuleName}};
{{end}}
    ...
    public(friend) fun get_signer_self(): signer acquires Authority {
        assert!(
            exists<Authority>(@{{.Address}}),
            E_NOT_SELF_SIGNED,
        );

        let auth = borrow_global<Authority>(@{{.Address}});

        let auth_signer = account::create_signer_with_capability(&auth.signer_capability);

        assert!(
            signer::address_of(&auth_signer) == @{{.Address}},
            E_CANNOT_SIGN_FOR_OTHER,
        );
