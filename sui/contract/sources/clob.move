...
module aux::clob {
    ...
    struct OrderFillEvent<phantom B, phantom Q> has copy, store, drop {
        order_id: u128,
        client_order_id: u128,
        is_bid: bool,
        owner: address,
    ...
    struct OrderCancelEvent<phantom B, phantom Q> has copy, store, drop {
        order_id: u128,
        client_order_id: u128,
        owner: address,
        cancel_qty: u64,
    ...
    struct OrderPlacedEvent<phantom B, phantom Q> has copy, store, drop {
        order_id: u128,
        client_order_id: u128,
        owner: address,
        is_bid: bool,
    ...
    public entry fun create_market<B, Q>(
        ...
        // Check if market was already created
        let type_name = std::type_name::get<Market<B, Q>>();
        assert!(!vec_set::contains(&registry.markets, &type_name), E_MARKET_ALREADY_EXISTS);

        // TODO: refactor volume tracking
    ...
    public entry fun place_order<B, Q>(
        ...
        // TODO: can we support trade delegation? Probably not if we require vault to be passed in, unless we store vault accounts as child objects
        // vault::assert_trader_is_authorized_for_account(tx_context::sender(ctx), vault_account_owner);
        ...
        if (base_au != 0 && quote_au != 0) {
            ...
            if (is_bid) {
                ...
                vault::decrease_user_balance<Q>(vault, vault_account_owner, (quote_au as u128));
                vault::increase_user_balance<B>(vault, vault_account_owner, (base_au as u128));
            } else {
                // taker receives quote, pays base
                vault::increase_user_balance<Q>(vault, vault_account_owner, (quote_au as u128));
                vault::decrease_user_balance<B>(vault, vault_account_owner, (base_au as u128));
            }
    ...
    fun handle_fill<B, Q>(
        ...
        if (taker_is_bid) {
            ...
            vault::increase_user_balance<Q>(vault, maker, (quote_qty + maker_rebate as u128));
            vault::decrease_unavailable_balance<B>(vault, maker, (base_qty as u128));
        } else {
            // maker pays quote + fee, receives base
            vault::increase_available_balance<Q>(vault, maker, (quote_qty as u128));
            vault::decrease_user_balance<Q>(vault, maker, (quote_qty - maker_rebate as u128));
            vault::increase_user_balance<B>(vault, maker, (base_qty as u128));
        ...
        if (!ZERO_FEES) {
            vault::increase_user_balance<Q>(vault, @aux, (taker_fee - maker_rebate as u128));
        };
        ...
        if (maker_remaining_qty == 0) {
            ...
            let order_idx = critbit::find(&open_orders.open_orders, maker_order.id);
    ...
    fun handle_placed_order<B, Q>(
        ...
        let placed_order_owner = order.owner;
        let vault_account_owner = open_orders.owner;
        assert!(placed_order_owner == vault_account_owner, E_INVALID_STATE);

        let qty = order.quantity;
        let price = order.price;
        let placed_quote_qty = quote_qty(price, qty, market.base_decimals);

        if (order.is_bid) {
            vault::decrease_available_balance<Q>(vault, vault_account_owner, (placed_quote_qty as u128));
        } else {
        ...
        critbit::insert(&mut open_orders.open_orders, order.id, OpenOrderInfo {
            is_bid: order.is_bid,
            price: order.price,
        });
    }
    ...
    fun new_order<B, Q>(
        ...
        if (order_type == FILL_OR_KILL) {
            ...
            if (critbit::size(side) == 0) {
                // Just emit the event
                event::emit<OrderCancelEvent<B, Q>>(
                    OrderCancelEvent<B, Q> {
                        order_id,
                        owner: order_owner,
                        cancel_qty: quantity,
            ...
            while(idx != CRITBIT_NULL_INDEX && filled < quantity) {
                ...
                if (can_fill) {
                    ...
                    while (order_idx != CRITBIT_NULL_INDEX && filled < quantity) {
                        ...
                        if (passive_order.owner == order_owner) {
                            if (stp_action_type == CANCEL_AGGRESSIVE) {
                                ...
                                event::emit<OrderCancelEvent<B, Q>>(
                                    OrderCancelEvent {
                                        ...
                                        owner: order_owner,
                                        cancel_qty: quantity,
                                        client_order_id, // this is same given the invariant
                                        order_type, // this is same given the invariant
                                    }
                                );
                                return (0,0)
                                ...
                                // TODO
                                process_cancel_order<B, Q>(vault, open_orders, cancelled, (lot_size as u128), market.base_decimals);
                                // Just emit the event
                                event::emit<OrderCancelEvent<B, Q>>(
                                    OrderCancelEvent {
                                        order_id,
        ...
        if (
            ...
            (order_type == PASSIVE_JOIN
                && !passive_join(market, &mut limit_price, is_bid, ticks_to_slide, direction_aggressive))
            ...
            || (order_type == POST_ONLY
                && !post_only<B, Q>(market, &mut limit_price, is_bid, ticks_to_slide))
        ) {
            // Just emit the event
            event::emit<OrderCancelEvent<B, Q>>(
                OrderCancelEvent {
        ...
        if (order.quantity > 0) {
            ...
            if (order_type == IMMEDIATE_OR_CANCEL) {
                ...
                handle_placed_order(vault, market, &order, open_orders);
    ...
    fun process_cancel_without_open_order_account<B, Q>(
        ...
        let event = OrderCancelEvent<B, Q> {
        ...
        if (cancelled.is_bid) {
            vault::increase_available_balance<Q>(
                vault,
                cancelled.owner,
                (quote_qty(cancelled.price, cancel_qty, base_decimals) as u128),
            );
        } else {
    ...
    /// Modifies order price by minimum number of ticks so that it will not fill, without exceeding `ticks_to_slide` and returns true.
    /// If impossible to keep order from filling, returns false
    fun post_only<B, Q>(market: &Market<B, Q>, order_price: &mut u64, is_bid: bool, ticks_to_slide: u64): bool {
        let tick_size = market.tick_size;
        let start_price = *order_price;
        if (is_bid){
            ...
            if (start_price < level_price) {
                return true
            };
            // TODO: confirm - 1 tick should be decrement 1 tick
            if ((start_price - level_price + tick_size) <= ticks_to_slide * tick_size) {
                assert!(level_price - tick_size > 0, E_SLIDE_TO_ZERO_PRICE);
                *order_price = level_price - tick_size;
                return true
            };
            return false
        } else{
            ...
            if ((level_price - start_price + tick_size) <= ticks_to_slide * tick_size) {
                *order_price = level_price + tick_size;
                return false
            };
            return false
    ...
    /// Calculate passive join price given ticks to slide and direction.
    /// If calculated price does not violate limit price, update order price and return true, otherwise return false.
    fun passive_join<B, Q>(market: &Market<B, Q>, order_price: &mut u64, is_bid: bool, ticks_to_slide: u64, direction_aggressive: bool) : bool {
        let tick_size = market.tick_size;
        let limit_price = *order_price;
        let pj_price;
        if (is_bid){
            ...
            if(direction_aggressive){
                pj_price = best_bid_level_price + ticks_to_slide * tick_size;
                if (pj_price >= best_ask_level_price) {
                    pj_price = best_ask_level_price - tick_size;
                }
            } else {
                pj_price = best_bid_level_price - ticks_to_slide * tick_size;
            };
            if (pj_price <= limit_price) {
                *order_price = pj_price;
                return true
            };
            return false
        } else{
            ...
            if(direction_aggressive){
                pj_price = best_ask_level_price - ticks_to_slide * tick_size;
                if (pj_price <= best_bid_level_price) {
                    pj_price = best_bid_level_price + tick_size;
                }
            }else{
                pj_price = best_ask_level_price + ticks_to_slide * tick_size;
            };
        ...
        while (!critbit::empty(side) && taker_order.quantity > 0) {
            ...
            if (
                ...
                while (level.total_quantity > 0 && taker_order.quantity > 0) {
                    ...
                    if (taker_order.owner == maker_order.owner) {
