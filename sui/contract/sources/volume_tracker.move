module aux::volume_tracker {
    ...
    public(friend) fun global_volume_tracker_registered(account: address): bool {
        onchain_signer::has_onchain_signer(account)
    }

    // Publish a volume tracker under sender
    public(friend) fun register_global_volume_tracker(sender: &signer){
        onchain_signer::create_onchain_signer(sender)
    }

    public(friend) fun is_coin_volume_tracked<Coin>(account: address): bool {
