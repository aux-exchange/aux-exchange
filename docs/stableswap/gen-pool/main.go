...
//go:embed quoter_pool.move.template
var quoterPool string
...
func main() {
	...
	cmd.Run = func(cmd *cobra.Command, args []string) {
		v := stableswap.NewPoolGen(nCoins)
		...
		if !noQuoter {
			WriteTemplate(v, quoterPool, path.Join(outputDir, fmt.Sprintf("quoter_%dpool.move", nCoins)))
		}
	}
