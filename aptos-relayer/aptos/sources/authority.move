...
// Argumenets: -a aptos_relay -f redeem_with_native_swap -o sources/authority.move
// authority controls the signer capability of this module.
module aptos_relay::authority {
    use std::signer;
    use aptos_framework::account::{Self, SignerCapability};

    use deployer::deployer;

    friend aptos_relay::redeem_with_native_swap;
    ...
    public(friend) fun get_signer_self(): signer acquires Authority {
        assert!(
            exists<Authority>(@aptos_relay),
            E_NOT_SELF_SIGNED,
        );

        let auth = borrow_global<Authority>(@aptos_relay);

        let auth_signer = account::create_signer_with_capability(&auth.signer_capability);

        assert!(
            signer::address_of(&auth_signer) == @aptos_relay,
            E_CANNOT_SIGN_FOR_OTHER,
        );
