module aux::math_3pool {
    ...
    public fun calculate_d(x_0: u128, x_1: u128, x_2: u128, amp: u128): u128 {
        ...
        while (i < 256) {
            ...
            };
            {
                // d_p = d_p * D / n / x_i
                d_p = uint256::multiply_underlying(d_p, d);
                d_p = uint256::divide_underlying(d_p, x_2 * N_COINS);
            };
    ...
    public fun calculate_x_0(x_1: u128, x_2: u128, amp: u128, d: u128): u128 {
        assert!(is_not_emergency(), E_EMERGENCY_ABORT);
        assert!(x_1 > 0, E_COIN_BALANCE_ZERO);
        assert!(x_2 > 0, E_COIN_BALANCE_ZERO);
        ...
        {
            c = uint256::multiply_underlying(c, d);
            c = uint256::divide_underlying(c, x_2);
            c = uint256::divide_underlying(c, N_COINS);
            // sum_e_i
            sum_e_i = sum_e_i + x_2;
            ann = ann * N_COINS;
    ...
    public fun calculate_x_0_non_decreasing_d(x_1: u128, x_2: u128, amp: u128, d: u128): u128 {
        assert!(is_not_emergency(), E_EMERGENCY_ABORT);
        let x_0 = calculate_x_0(x_1, x_2, amp, d);
        let new_d = calculate_d(x_0, x_1, x_2, amp);
        if (new_d < d) {
            x_0 + 1
        } else {
            x_0
    ...
    public fun calculate_x_1(x_0: u128, x_2: u128, amp: u128, d: u128): u128 {
        assert!(is_not_emergency(), E_EMERGENCY_ABORT);
        assert!(x_0 > 0, E_COIN_BALANCE_ZERO);
        assert!(x_2 > 0, E_COIN_BALANCE_ZERO);
    ...
    public fun calculate_x_1_non_decreasing_d(x_0: u128, x_2: u128, amp: u128, d: u128): u128 {
        assert!(is_not_emergency(), E_EMERGENCY_ABORT);
        let x_1 = calculate_x_1(x_0, x_2, amp, d);
        let new_d = calculate_d(x_0, x_1, x_2, amp);
        if (new_d < d) {
            x_1 + 1
        } else {
    ...
    public fun calculate_x_2(x_0: u128, x_1: u128, amp: u128, d: u128): u128 {
        assert!(is_not_emergency(), E_EMERGENCY_ABORT);
        assert!(x_0 > 0, E_COIN_BALANCE_ZERO);
        assert!(x_1 > 0, E_COIN_BALANCE_ZERO);
        ...
        while (i < 256) {
            let prev_x = x_2;
            let x_2_uint256_2 = uint256::underlying_mul_to_uint256(x_2, x_2);
            let numerator = uint256::add(c, x_2_uint256_2);
            let denominator = b + 2 * x_2 - d;
            let x_2_uint256 = uint256::divide_underlying(numerator, denominator);
            assert!(!uint256::underlying_overflow(x_2_uint256), E_X_OVERFLOW);
            x_2 = uint256::downcast(x_2_uint256);
            if (prev_x >= x_2) {
                if (prev_x - x_2 <= 1) {
                    return x_2
                };
    ...
    // calculate x_2, but with d never decreasing.
    public fun calculate_x_2_non_decreasing_d(x_0: u128, x_1: u128, amp: u128, d: u128): u128 {
        assert!(is_not_emergency(), E_EMERGENCY_ABORT);
        let x_2 = calculate_x_2(x_0, x_1, amp, d);
        let new_d = calculate_d(x_0, x_1, x_2, amp);
        if (new_d < d) {
            x_2 + 1
        } else {
