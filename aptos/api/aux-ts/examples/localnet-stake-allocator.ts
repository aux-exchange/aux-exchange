...
async function calculateOptimalPositions(poolIds: PoolRecords): Promise<PoolRecords> {
    let poolClient: StakePoolClient = new StakePoolClient(auxClient, {
        coinInfoReward: await auxClient.getCoinInfo(usdcCoin),
        coinInfoStake: await auxClient.getCoinInfo(btcCoin),
    });
    ...
    let x1 = 0.5 * (r1*t2*(M+s2-p2) + r2*t1*(M-s1+p1)) / (r1*t2 + r2*t1);
    x1 = Math.min(x1, M);
    x1 = Math.max(x1, 0);
    let x2 = M - x1;

    let poolRewardPerShareAnn1 = 0., poolRewardPerShareAnn2 = 0.;
    if (x1 > 0) {poolRewardPerShareAnn1 = r1 * x1 / (s1 - p1 + x1 ) / t1  * oneYearUs;}
    if (x2 > 0) {poolRewardPerShareAnn2 = r2 * x2 / (s2 - p2 + x2 ) / t2  * oneYearUs;}
    let poolRewardPerShareAnnTotal = poolRewardPerShareAnn1 + poolRewardPerShareAnn2;

    console.log(">>>> New optimal pool positions: %f BTC in pool1 and %f BTC in pool2", x1, x2);
    console.log(">>>> Estimated future reward per share (annauzlied): %f USDC", poolRewardPerShareAnnTotal);
    return {"p1": x1, "p2": x2};
}

// calculate trades as (optimalPositions - currentPositions)
async function calculateTrades(currentPositions: PoolRecords, optimalPositions: PoolRecords): Promise<PoolRecords> {
    let trades: PoolRecords = {
        "p1": optimalPositions.p1 - currentPositions.p1,
        "p2": optimalPositions.p2 - currentPositions.p2
    };
    console.log(">>>> Suggested trades: %f BTC in pool1 and %f BTC in pool2", trades.p1, trades.p2);
    return trades
}
...
async function placeTrades(poolIds: PoolRecords): Promise<void> {
    ...
    console.log(">>>> poo1 %d AmountStaked %f BTC RewardRemaining %f USDC",
     pool1.poolId, pool1.amountStaked, pool1.rewardRemaining);
    console.log(">>>> poo2 %d AmountStaked %f BTC RewardRemaining %f USDC",
     pool2.poolId, pool2.amountStaked, pool2.rewardRemaining);

    const currentPositions = await queryPoolPositions(poolIds);
    console.log(">>>> Current staker pool positions: %f BTC in pool1 and %f BTC in pool2", 
    currentPositions.p1, currentPositions.p2);
    ...
    try {
        if (Trades.p1 < 0 && Trades.p2 > 0) {
            console.log(">>>> Withdrawing %f BTC from pool1, depositing %f BTC to pool2", 
            Math.abs(Trades.p1), Trades.p2);
            ...
            console.log(">>>> Withdrawing %f BTC from pool2, depositing %f BTC to pool1", 
            Math.abs(Trades.p2), Trades.p1);
            await poolClient.withdraw({
            ...
            console.log(">>>> Depositing %f BTC from pool1, depositing %f BTC to pool2", 
            Trades.p1, Trades.p2);
            await poolClient.deposit({
                amount: DU(Trades.p1),
                poolId: poolId1});
...
async function stakeAllocator(): Promise<void> {
    ...
    console.log(">>>> Created two stake pools with ids %d and %d", poolIds.p1, poolIds.p2);
    ...
    try {
        await poolClient.deposit({
            amount: DU(9),
            poolId: poolIds.p1}, {"sender" : stakerOthers})
    } catch (e) {
        "  Other stakers failed to deposit into pool1"
    }
