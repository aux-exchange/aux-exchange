...
describe("Stable 2pool tests", function () {
  this.timeout(30000);
  ...
  */
  ...
  const poolClient: StableSwapClient = new StableSwapClient(auxClient, [usdcCoin, usdtCoin]);
  let pool: StableSwap;
  // let vault: Vault;
  ...
  it("addLiquidity", async () => {
    {
      ...
      const [expectedUSDCAmount, expectedUSDTAmount, expectedLPAmount] = [1_000, 4_000, 499_181];
  ...
  it("swap", async () => {
    ...
    const [expectedUSDCAmount, expectedUSDTAmount, expectedLPAmount] = [1002, 3998, 499_181];
    pool = await poolClient.query();
    const [usdcAmount, usdtAmount] = pool.amounts;
    const [usdcInfo, usdtInfo] = pool.coinInfos;
    assert.equal(
