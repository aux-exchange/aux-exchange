...
export async function pool(
  ...
  const coinTypeLP = `${auxClient.moduleAddress}::stable_swap_2pool::LP<${coinTypeX}, ${coinTypeY}>`;
...
export async function positions(
  ...
  const positionResources = resources.filter(
    (resource) =>
      resource.type.includes("stable_swap_2pool::LP") &&
...
function poolType(moduleAddress: Types.Address, poolInput: PoolInput) {
  return `${moduleAddress}::stable_swap_2pool::Pool<${poolInput.coinTypeX}, ${poolInput.coinTypeY}>`;
}
