...
export default class Pool implements core.query.Pool {
  ...
  static async create(
    auxClient: AuxClient,
    { sender, coinTypeX, coinTypeY, feePct, startA, xRampUpDec, yRampUpDec }: CreateParams,
    ...
    await core.mutation.createPool(
      auxClient,
      {
        ...
        startA: startA.toString(),
        xRampUp: Math.pow(10, xRampUpDec).toString(),
        yRampUp: Math.pow(10, yRampUpDec).toString(),
      },
      transactionOptions
    );
   ...
   */
  async addLiquidity(
    { sender, amountX, amountY }: AddLiquidityParams,
    transactionOptions?: TransactionOptions
    ...
    const tx = await this.auxClient.generateSignSubmitWaitForTransaction({
      sender,
      payload: {
        function: `${this.auxClient.moduleAddress}::stable_swap_2pool::add_liquidity`,
        type_arguments: [this.coinInfoX.coinType, this.coinInfoY.coinType],
        arguments: [amountAuX, amountAuY],
      },
      transactionOptions,
    });
    if (tx.success) {
  ...
  async addApproximateLiquidity(
    ...
    return this.auxClient
      .generateSignSubmitWaitForTransaction({
        sender,
        payload: {
          function: `${this.auxClient.moduleAddress}::stable_swap_2pool::add_approximate_liquidity`,
          type_arguments: [this.coinInfoX.coinType, this.coinInfoY.coinType],
          arguments: [
...
interface CreateParams {
  ...
  feePct: number;
  startA: number;
  xRampUpDec: number; // the decimal coin X need to be ramped up
  yRampUpDec: number; // the decimal coi Y need to be ramped up
}
