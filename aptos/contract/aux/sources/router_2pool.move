...
module aux::router_2pool {
    ...
    use aux::stable_2pool::{Self, LPBundle, bundle_melt, bundle_extract, bundle_value};

    struct LPBundleStore<phantom Coin0, phantom Coin1> has key {
        lp_bundle: LPBundle<Coin0, Coin1>,
    }

    public entry fun deposit<Coin0, Coin1>(sender: &signer, lp_bundle: LPBundle<Coin0, Coin1>) acquires LPBundleStore {
        let sender_addr = signer::address_of(sender);
        if (!exists<LPBundleStore<Coin0, Coin1>>(sender_addr)) {
            move_to(
                sender,
                LPBundleStore {
                    lp_bundle,
            ...
            let store = borrow_global_mut<LPBundleStore<Coin0, Coin1>>(sender_addr);
            bundle_melt(&mut store.lp_bundle, lp_bundle)
        }
    }

    public fun withdraw<Coin0, Coin1>(sender: &signer, amount: u64): LPBundle<Coin0, Coin1> acquires LPBundleStore {
        let sender_addr = signer::address_of(sender);
        bundle_extract(&mut borrow_global_mut<LPBundleStore<Coin0, Coin1>>(sender_addr).lp_bundle, amount)
    }

    public fun balance<Coin0, Coin1>(sender_addr: address): u64 acquires LPBundleStore {
        if (!exists<LPBundleStore<Coin0, Coin1>>(sender_addr)) {
            0
        } else {
            bundle_value(&borrow_global<LPBundleStore<Coin0, Coin1>>(sender_addr).lp_bundle)
        }
    }
