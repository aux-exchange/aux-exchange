...
module aux::quoter_3pool {
    ...
    struct LPBundle has store, copy, drop {
        ...
        reward_token_2: QuoterRedeemToken,
    }
    ...
    struct Quoter has store, copy, drop {
        ...
        reward_distributor_2: RewardDistributionQuoter,
    ...
    public fun new_quoter(
        ...
        Quoter {
            ...
            reserve_2: quote_coin::new(reserve_2),
            fee_2: quote_coin::new(fee_2),
            scaler_2,
    ...
    public fun move_fee_into_reward_distributor(pool: &mut Quoter)
    {
        ...
        let fee_moved_2 = quote_coin::value(&pool.fee_2);
        // only merge if there is fee to move
        if (fee_moved_2> 0) {
            fee_added = true;
            reward_quoter::add_reward(&mut pool.reward_distributor_2, quote_coin::extract_all(&mut pool.fee_2));
        };
...
pool: &mut Quoter,
    ...
    ): LPBundle
    {
        ...
        quote_coin::merge(&mut pool.reserve_2, amount_2);
        ...
        let reward_token_2 = reward_quoter::mint(&mut pool.reward_distributor_2, lp_to_mint);
...
pool: &mut Quoter,
    ...
    ): (u64, u64, u64, LPBundle)
    {
        ...
        let coin_2 = if (amount_2_to_withdraw > 0) {
            ...
            let fee_coin_2 = quote_coin::extract(&mut pool.reserve_2, fee);
            quote_coin::merge(&mut pool.fee_2, fee_coin_2);

            quote_coin::extract(&mut pool.reserve_2, amount_2_to_withdraw)
        } else {
            quote_coin::zero()
        };
        ...
        // burn the reward 2
        let reward_coin_2 = reward_quoter::burn(&mut pool.reward_distributor_2, reward_token_2);
        quote_coin::merge(&mut coin_2, reward_coin_2);
...
pool: &mut Quoter,
    ...
    ): (u64, u64, u64)
    {
        ...
        let before_reserve_2 = quote_coin::value(&pool.reserve_2);

        let withdraw_2 = (
            ((before_reserve_2 as u128) * (lp_burnt as u128))
            / before_lp_coins_supply
             as u64);
        let coin_2 = quote_coin::extract(&mut pool.reserve_2, withdraw_2);
        let reward_coin_2 = reward_quoter::burn(&mut pool.reward_distributor_2, reward_token_2);
        quote_coin::merge(&mut coin_2, reward_coin_2);
...
pool: &mut Quoter,
    ...
    ): (u64, u64, u64)
    {
        ...
        if (out_coin_index != 2) {
            quote_coin::merge(&mut pool.reserve_2, quote_coin::extract_all(&mut coin_2));
        };

        let after_reserve_2 = quote_coin::value(&pool.reserve_2);
        ...
        if (is_coin_2_out) {
            ...
            let fee_2 = (fee_amount as u64);
            // amount the user will receive
            let amount_2_out = diff - fee_2;

            assert!(amount_2_out > 0, E_OUTPUT_COIN_UNDERFLOW);
            assert!(amount_2_out >= min_quantity_out, E_OUTPUT_COIN_INSUFFICIENT);

            let out_coin = quote_coin::extract(&mut pool.reserve_2, amount_2_out);
            quote_coin::merge(&mut coin_2, out_coin);
            let fee_coin = quote_coin::extract(&mut pool.reserve_2, fee_2);
            quote_coin::merge(&mut pool.fee_2, fee_coin);
        };
...
pool: &mut Quoter,
    ...
    ): (u64, u64, u64)
    {
        ...
        if (is_coin_2_out && requested_quantity_2 > 0) {
            ...
            let fee = (fee as u64);
            let fee_coin = quote_coin::extract(&mut pool.reserve_2, fee);
            quote_coin::merge(&mut pool.fee_2, fee_coin);
            quote_coin::merge(&mut coin_2, quote_coin::extract(&mut pool.reserve_2, requested_quantity_2));
        };
        ...
        if (in_coin_index == 2) {
            ...
            let amount_2_in = after_reserve_2 - before_reserve_2;
            assert!(amount_2_in <= quote_coin::value(&coin_2), E_INPUT_COIN_INSUFFICIENT);
            quote_coin::merge(&mut pool.reserve_2, quote_coin::extract(&mut coin_2, amount_2_in));
        };
    ...
    public fun bundle_extract(lp: &mut LPBundle, amount: u64): LPBundle {
        LPBundle {
            ...
            reward_token_2: reward_quoter::token_extract(&mut lp.reward_token_2, amount),
    ...
    public fun bundle_attach_reward(
        ...
        assert!(
            quote_coin::value(&lps) == reward_quoter::token_value(&reward_token_2),
            E_REWARD_TOKEN_AMOUNT_MISMATCH
        );
    ...
    public fun bundle_is_fungible(
        ...
        if (!reward_quoter::is_fungible(&l.reward_token_2, &r.reward_token_2)) {
            return false
        };
    ...
    public fun bundle_merge(
        ...
        reward_quoter::token_merge(&mut l.reward_token_2, reward_token_2);
    }
    ...
    public fun bundle_melt(
        ...
        reward_quoter::token_melt(&mut l.reward_token_2, reward_token_2);
    }
    ...
    public fun update_balanced_reserve(pool: &mut Quoter) {
        ...
        let reserve_2_unscaled = quote_coin::value(&pool.reserve_2);
    ...
    public fun get_reserve_2(pool: &Quoter): u64 {
        quote_coin::value(&pool.reserve_2)
    }

    public fun get_fee_2(pool: &Quoter): u64 {
        quote_coin::value(&pool.fee_2)
    }
}
