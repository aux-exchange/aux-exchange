...
module aux::stable_2pool_test {
    ...
    fun au_out<CoinIn, CoinOut>(au_in: u64): u64 {
        if (stable_2pool::pool_exists<CoinIn, CoinOut>()) {
            ...
            let (_, out) = quoter_2pool::swap_exact_coin_for_coin(&mut quoter, au_in, 0, 1, 0);
            out
        } else if(stable_2pool::pool_exists<CoinOut, CoinIn>()) {
            let quoter = stable_2pool::get_quoter<CoinOut, CoinIn>();
            let (out, _) = quoter_2pool::swap_exact_coin_for_coin(&mut quoter, 0, au_in, 0, 0);
            out
        } else {
            abort(ETEST_FAILED)
        }
    ...
    fun au_in<CoinIn, CoinOut>(au_out: u64): u64 {
        if (stable_2pool::pool_exists<CoinIn, CoinOut>()) {
            let quoter = stable_2pool::get_quoter<CoinIn, CoinOut>();
            let (remaining, _) = quoter_2pool::swap_coin_for_exact_coin(&mut quoter, 0, au_out, 0, MAX_U64);
            MAX_U64 - remaining
        } else if(stable_2pool::pool_exists<CoinOut, CoinIn>()) {
            let quoter = stable_2pool::get_quoter<CoinOut, CoinIn>();
            let (_, remaining) = quoter_2pool::swap_coin_for_exact_coin(&mut quoter, au_out, 0, 1, MAX_U64);
            MAX_U64 - remaining
        } else {
            abort(ETEST_FAILED)
        }
    ...
    public fun test_add_liquidity(sender: &signer, aptos_framework: &signer) {
        ...
        assert!(coin::balance<stable_2pool::LP<FakeCoin<USDC>, FakeCoin<USDT>>>(sender_addr) == 998362,
                coin::balance<stable_2pool::LP<FakeCoin<USDC>, FakeCoin<USDT>>>(sender_addr));
    }
    ...
    public fun test_add_liquidity_small(sender: &signer, aptos_framework: &signer) {
        ...
        assert!(coin::balance<stable_2pool::LP<FakeCoin<USDC>, FakeCoin<USDT>>>(sender_addr) == 200000,
                coin::balance<stable_2pool::LP<FakeCoin<USDC>, FakeCoin<USDT>>>(sender_addr));
    }
    ...
    fun test_reset_pool(sender: &signer, liquidity_provider: &signer, aptos_framework: &signer) {
        ...
        assert!(
            coin::balance<stable_2pool::LP<FakeCoin<USDC>, FakeCoin<USDT>>>(lp_addr) == 499181,
            coin::balance<stable_2pool::LP<FakeCoin<USDC>, FakeCoin<USDT>>>(lp_addr)
        );
    ...
    fun test_add_liquidity_remove_liquidity(sender: &signer, aptos_framework: &signer) {
        ...
        assert!(coin::balance<stable_2pool::LP<FakeCoin<USDC>, FakeCoin<USDT>>>(sender_addr) == 494181,
                ETEST_FAILED);
        ...
        assert!(coin::balance<FakeCoin<USDC>>(sender_addr) == aux_bal, ETEST_FAILED);
        assert!(coin::balance<FakeCoin<USDT>>(sender_addr) == test_bal, ETEST_FAILED);
        assert!(coin::balance<stable_2pool::LP<FakeCoin<USDC>, FakeCoin<USDT>>>(sender_addr) == 492481,
                ETEST_FAILED);
    ...
    fun test_swap_helper<CoinX, CoinY, CoinIn, CoinOut>(
        ...
        stable_2pool::delete_pool_in_test<CoinX, CoinY>(sender);
    }
    ...
    public fun test_add_liquidity_with_slippage_tolerance(
        ...
        assert!(coin::balance<stable_2pool::LP<FakeCoin<USDC>, FakeCoin<USDT>>>(sender_addr) == 499281,
                coin::balance<stable_2pool::LP<FakeCoin<USDC>, FakeCoin<USDT>>>(sender_addr) );
        ...
        let sender_lp = coin::balance<stable_2pool::LP<FakeCoin<USDC>, FakeCoin<USDT>>>(sender_addr);
    ...
    public fun test_stable_curve_frax(
        ...
        assert!(coin::balance<stable_2pool::LP<FakeCoin<USDC>, FakeCoin<USDT>>>(sender_addr) == 599987504945812,
                coin::balance<stable_2pool::LP<FakeCoin<USDC>, FakeCoin<USDT>>>(sender_addr) );
    ...
    public fun test_stable_curve_steth(
        ...
        assert!(coin::balance<stable_2pool::LP<FakeCoin<USDC>, FakeCoin<USDT>>>(sender_addr) == 119983034036,
                coin::balance<stable_2pool::LP<FakeCoin<USDC>, FakeCoin<USDT>>>(sender_addr));
