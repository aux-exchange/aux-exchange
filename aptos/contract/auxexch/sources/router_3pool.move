...
module aux::router_3pool {
    ...
    use aux::stable_3pool::{Self, LPBundle, LP, bundle_detach_reward, bundle_attach_reward};
    ...
    struct LPRewardTokenStore<phantom Coin0, phantom Coin1, phantom Coin2> has key {
    ...
    public entry fun deposit<Coin0, Coin1, Coin2>(sender: &signer, lp_bundle: LPBundle<Coin0, Coin1, Coin2>) acquires LPRewardTokenStore {
        let (lps,
            reward_token_0,
            reward_token_1,
            reward_token_2,
        ...
        if (!exists<LPRewardTokenStore<Coin0, Coin1, Coin2>>(sender_addr)) {
            move_to(
                sender,
                LPRewardTokenStore {
                    reward_token_0,
            ...
            let store = borrow_global_mut<LPRewardTokenStore<Coin0, Coin1, Coin2>>(sender_addr);
            ...
            reward_distributor::token_melt(&mut store.reward_token_2, reward_token_2);
        };
        if (!coin::is_account_registered<LP<Coin0, Coin1, Coin2>>(sender_addr)) {
            coin::register<LP<Coin0, Coin1, Coin2>>(sender);
        };
        coin::deposit(sender_addr, lps);
    }

    public fun withdraw<Coin0, Coin1, Coin2>(sender: &signer, amount: u64): LPBundle<Coin0, Coin1, Coin2> acquires LPRewardTokenStore {
        let sender_addr = signer::address_of(sender);
        let lps = coin::withdraw<LP<Coin0, Coin1, Coin2>>(sender, amount);
        let reward_store = borrow_global_mut<LPRewardTokenStore<Coin0, Coin1, Coin2>>(sender_addr);
        ...
        let reward_token_2 = reward_distributor::token_extract(&mut reward_store.reward_token_2, amount);
    ...
    public fun balance<Coin0, Coin1, Coin2>(sender_addr: address): u64 acquires LPRewardTokenStore {
        if (!exists<LPRewardTokenStore<Coin0, Coin1, Coin2>>(sender_addr)) {
            0
        } else {
            reward_distributor::token_value(&borrow_global<LPRewardTokenStore<Coin0, Coin1, Coin2>>(sender_addr).reward_token_0)
        }
    }
    ...
    public entry fun create_pool<Coin0, Coin1, Coin2>(sender: &signer, fee_numerator: u128, amp: u128) {
        stable_3pool::create_pool<Coin0, Coin1, Coin2>(sender, fee_numerator, amp);
    }

    public entry fun update_amp<Coin0, Coin1, Coin2>(sender: &signer, amp: u128) {
        stable_3pool::update_amp<Coin0, Coin1, Coin2>(sender, amp);
    }

    public entry fun update_fee<Coin0, Coin1, Coin2>(sender: &signer, fee_numerator: u128) {
        stable_3pool::update_fee<Coin0, Coin1, Coin2>(sender, fee_numerator);
    }
    ...
    public entry fun remove_liquidity<Coin0, Coin1, Coin2>(
        ...
        let (coin_0, coin_1, coin_2) = stable_3pool::remove_liquidity<Coin0, Coin1, Coin2>(
            sender_addr,
            lp,
        );
